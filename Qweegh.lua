-- ============================================
-- БОЛЬШАЯ ТЮРЬМА + ПОБЕГ С ШАРИКАМИ (FINAL)
-- ДЛЯ DELTA MOBILE
-- ============================================
...
-- ============================================
-- ФУНКЦИЯ УДАЛЕНИЯ ТЮРЬМЫ
-- ============================================

local function destroyPrison()
    for _, part in ipairs(prisonParts) do
...
Btn2.Position = UDim2.new(0.1, 0, 0.50, 0)
...
Btn3.Position = UDim2.new(0.1, 0, 0.72, 0)
...
-- ============================================
-- ФУНКЦИЯ СОЗДАНИЯ ТЮРЬМЫ
-- ============================================

local function createPrison(seconds)
    ...
    -- Позиция тюрьмы (над землёй)
    local prisonPos = RootPart.Position + Vector3.new(0, 3, 0)
    
    -- Телепортируем ВНУТРЬ клетки
    ...
    for _, pos in ipairs(wallPositions) do
        ...
        wall.Size = Vector3.new(thickness, cageSize, cageSize)
        ...
        table.insert(prisonParts, wall)
    end
    
    -- Пол
    local floor = Instance.new("Part")
    floor.Size = Vector3.new(cageSize, thickness, cageSize)
    floor.Position = prisonPos + Vector3.new(0, -cageSize/2 + thickness/2, 0)
    floor.Anchored = true
    ...
    table.insert(prisonParts, floor)
    
    -- Потолок
    local roof = floor:Clone()
    roof.Position = prisonPos + Vector3.new(0, cageSize/2 - thickness/2, 0)
    roof.Parent = workspace
    table.insert(prisonParts, roof)
...
-- ============================================
-- ФУНКЦИЯ ПОБЕГА (ТЫ БЕЖИШЬ, ШАРИКИ ЗА ТОБОЙ)
-- ============================================
...
local function escapeWithBalls()
    ...
    print("💨 ПОБЕГ! Беги 10 секунд!")
    
    -- Удаляем старую тюрьму
    destroyPrison()
    
    -- Возвращаем движение
    Humanoid.WalkSpeed = oldSpeed + 10 -- Немного быстрее для побега
    Humanoid.JumpPower = 50
    Humanoid.PlatformStand = false
    ...
    label.Size = UDim2.new(1, 0, 1, 0)
    label.BackgroundTransparency = 1
    label.Text = "💨 ПОБЕГ! 10с"
    label.TextColor3 = Color3.fromRGB(0, 255, 0)
    label.TextSize = 30
    label.Font = Enum.Font.GothamBold
    label.Parent = bill
    
    -- СОЗДАЁМ ШАРИКИ (бегут по ЗЕМЛЕ)
    local balls = {}
    local caught = false
    
    -- 10 шариков вокруг игрока (по земле)
    for i = 1, 10 do
        ...
        local y = RootPart.Position.Y - 0.5 -- По земле!
        
        ball.Position = Vector3.new(x, y, z)
        ball.Parent = workspace
        table.insert(balls, ball)
        
        -- Движение шариков к игроку (по земле!)
        task.spawn(function()
            while ball and not caught do
                local dir = (Vector3.new(RootPart.Position.X, ball.Position.Y, RootPart.Position.Z) - ball.Position).Unit
                ball.Velocity = Vector3.new(dir.X * 4, 0, dir.Z * 4) -- Скорость 4
                task.wait(0.05)
            end
        end)
    end
    
    -- ТАЙМЕР 10 СЕКУНД (без остановки игрока!)
    local startTime = tick()
    
    repeat
        task.wait(0.1)
        
        -- Проверка: догнал ли шарик?
        for _, ball in ipairs(balls) do
            if ball and (Vector3.new(ball.Position.X, RootPart.Position.Y, ball.Position.Z) - RootPart.Position).Magnitude < 2.5 then
                caught = true
                break
            end
        end
    ...
    if caught then
        ...
        isEscaping = false
        isInPrison = false
        createPrison(10) -- Сажаем обратно
...
print("=========================================")
print("⚠️ БОЛЬШАЯ ТЮРЬМА (FINAL)")
print("✅ Ты ВНУТРИ клетки!")
print("✅ Побег: ТЫ БЕЖИШЬ + ШАРИКИ ЗА ТОБОЙ!")
print("✅ Шарики по земле! Скорость 4!")
print("✅ 10 секунд на побег!")
print("=========================================")
